...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, scaled=False, act='relu', scaling_fac=1.):
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, scaled=False, act='relu'):
        ...
        self.layer1 = self._make_layer(block, 1, _outputs[0], num_blocks[0], stride=1, scaled=scaled, act=act, scaling_fac=scaling_factor)
        self.layer2 = self._make_layer(block, 2, _outputs[1], num_blocks[1], stride=2, scaled=scaled, act=act, scaling_fac=scaling_factor)
        self.layer3 = self._make_layer(block, 3, _outputs[2], num_blocks[2], stride=2, scaled=scaled, act=act, scaling_fac=scaling_factor)
    ...
    def _make_layer(self, block, section_num, planes, num_blocks, stride, scaled, act, scaling_fac):
        strides = [stride] + [1]*(num_blocks-1)
        scaling_factors = 1+np.arange((section_num-1)*num_blocks, section_num*num_blocks)
        scaling_factors = scaling_factors * np.log(scaling_factors + 1)
        layers = []
        for i in range(num_blocks):
            layers.append(block(self.in_planes, planes, strides[i], scaled, act, scaling_fac=scaling_factors[i]))
...
def resnet(depth=32, dataset='cifar10', scaled=False, BatchNorm=False, act='relu'):
    ...
    if not BatchNorm:
        ...
        return ResNet(BasicBlock_BN, [n]*3, num_classes, scaled, act)
